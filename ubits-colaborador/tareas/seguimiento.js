...
(function () {
    ...
    if (modalsContainer) {
        ...
        if (typeof window.getDrawerHtml === 'function') {
            modalsContainer.innerHTML += window.getDrawerHtml({ overlayId: 'filtros-modal-overlay', title: 'Filtros', bodyHtml: filtrosBody, footerHtml: filtrosFooter, size: 'md', closeButtonId: 'filtros-modal-close' });
        }
    ...
    /**
     * Rango «Personalizado» del selector de período (no es un chip de «Filtros aplicados»).
     */
    function isSeguimientoPeriodoPersonalizado() {
        const f = currentFilters;
        return !f.periodo && !!(f.fechaCreacionDesde && f.fechaCreacionHasta);
    }
    ...
    function seguimientoHasAppliedFiltersExcludingSearch() {
        ...
        if (!isSeguimientoPeriodoPersonalizado() && (f.fechaCreacionDesde || f.fechaCreacionHasta)) return true;
    ...
    function renderFiltrosAplicados() {
        ...
        // Fechas de creación (modal; el período personalizado se muestra en el selector de período)
        if (!isSeguimientoPeriodoPersonalizado() && (currentFilters.fechaCreacionDesde || currentFilters.fechaCreacionHasta)) {
            hasFilters = true;
            let fechaText = '';
            if (currentFilters.fechaCreacionDesde && currentFilters.fechaCreacionHasta) {
