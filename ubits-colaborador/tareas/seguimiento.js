...
(function () {
    ...
    if (modalsContainer) {
        ...
        if (typeof getDatePickerModalHtml === 'function') {
            modalsContainer.innerHTML += getDatePickerModalHtml({
                mode: 'range',
            ...
            modalsContainer.innerHTML += getDatePickerModalHtml({
                ...
                singleFieldLabel: 'Nueva fecha de finalización',
    ...
    // Selector de fechas personalizado (components/date-picker-modal.js + createCalendar)
    function initDatePicker() {
        if (typeof window.createDatePickerModalController !== 'function') return;
        if (!document.getElementById('date-picker-overlay')) return;
        ...
        function restaurarPeriodoTrasCancelar() {
        ...
        var rangeCtrl = window.createDatePickerModalController({
            mode: 'range',
            overlayId: 'date-picker-overlay',
            calendarContainerId: 'date-picker-calendar',
            startInputId: 'date-picker-fecha-inicio',
            ...
            applyButtonId: 'date-picker-aplicar',
            closeButtonId: 'date-picker-close',
            getMaxDate: getSeguimientoCalendarMaxDate,
            defaultDisplayMonth: SEGUIMIENTO_DATE_PICKER_DEFAULT_MONTH,
            onCancel: restaurarPeriodoTrasCancelar,
            onApply: function (fechaInicio, fechaFin) {
                var fechaInicioStr = formatearFechaParaMostrar(fechaInicio);
                var fechaFinStr = formatearFechaParaMostrar(fechaFin);
                currentFilters.periodo = null;
        ...
        window.openDatePicker = function () {
            ...
            var end = null;
            if (currentFilters.fechaCreacionDesde && currentFilters.fechaCreacionHasta) {
                start = parseFecha(currentFilters.fechaCreacionDesde);
                end = parseFecha(currentFilters.fechaCreacionHasta);
            }
            rangeCtrl.open({ start: start, end: end });
        };
    }
    ...
    function initActionButtons() {
        ...
        // Cambiar fecha de vencimiento / finalización (date-picker-modal + createCalendar)
        ...
        var planFechaCtrl =
            typeof window.createDatePickerModalController === 'function' &&
            window.createDatePickerModalController({
                mode: 'single',
                overlayId: 'plan-fecha-overlay',
                calendarContainerId: 'plan-fecha-calendar',
                singleInputId: 'plan-fecha-input',
                ...
                getMaxDate: getSeguimientoCalendarMaxDate,
                defaultDisplayMonth: SEGUIMIENTO_DATE_PICKER_DEFAULT_MONTH,
                onApply: function (planFechaSelected) {
                    ...
                    if (activeTab === 'tareas') {
                        ...
                        updateIndicadores();
                        if (typeof showToast === 'function') {
                            showToast('success', 'Fecha de vencimiento actualizada para ' + selectedIds.size + ' tarea(s)');
                        }
                    } else {
                        selectedIds.forEach(function (id) {
                            var row = SEGUIMIENTO_DATA.find(function (r) { return r.id === id; });
                        ...
                        updateIndicadores();
                        if (typeof showToast === 'function') {
                            showToast('success', 'Fecha de finalización actualizada para ' + selectedIds.size + ' plan(es)');
                        }
                    }
                }
            });

        if (planFechaBtn && planFechaCtrl) {
            planFechaBtn.addEventListener('click', function () {
                if (selectedIds.size === 0) return;
                if (planFechaModalTitle) {
                    planFechaModalTitle.textContent =
                        activeTab === 'tareas' ? 'Cambiar fecha de vencimiento' : 'Cambiar fecha de finalización';
                }
                planFechaCtrl.open({});
            });
        }
