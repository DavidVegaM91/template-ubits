...
53. [x] **Task-strip: variante Aprendizaje (icono en tirilla y demo en Mi lista)**

    - **Componente (`components/task-strip.css`, `components/task-strip.js`):** Si la tarea trae **`taskType === 'aprendizaje'`** (o la tirilla se renderiza con `taskType: 'aprendizaje'`), la fila del título muestra a la izquierda el icono **`far fa-graduation-cap`** en color **brand** (`--ubits-accent-brand`), alineado con la primera línea cuando el título es multilínea (mismo criterio que task-detail: contenedor del icono con altura fija tipo tirilla, **desktop y móvil**). El título sigue dentro de **`.tarea-titulo-inner`** para que **edición inline** (`startInlineEditTaskName`) no elimine el icono.

    - **Mi lista (`tareas.js`) + BD (`bd-master/bd-tareas-y-planes.js`):** Tarea demo **Aprendizaje** para **María Alejandra Sánchez Pardo** en el día visitado, con nombre **«Curso: Trabajo en equipo y colaboración»**, plan **Metas personales…**, contenido **f012**, **`learningContentId`** y **`taskType: 'aprendizaje'`**. Id fijo para prototipo y URL compartible: **`TASK_ID_PROTO_APRENDIZAJE` = `9000000000001`** (`getTaskDetail` devuelve el detalle canónico; `task-detail.html?id=9000000000001` sin depender de `sessionStorage`).

    - **Scope del componente:** `components/task-strip.*`; consumo en la vista de tareas vía `renderTaskStrip` / datos de `tareas.js` y BD.
...
56. [x] **Mejoras en la vista de tareas: filtros y tareas sin fecha**

    Entregas en **`tareas.html`**, **`tareas.js`**, **`tareas.css`** (y componente **Attention badge** compartido con botones):

    - Contadores de **Vencidas** y **Sin fecha de vencimiento** con el componente oficial **`ubits-attention-badge`** (variante error, sm; círculo para 1–9 dígitos).
    - **Attention badge** en el botón **Filtros** cuando hay criterios aplicados.
    - Aire superior estable en la lista al ocultar secciones: **`padding-top`** en **`.tareas-scroll-container`** para que **Sin fecha de vencimiento** no quede pegada al módulo del calendario si **Vencidas** está oculta por filtros.

    **Vista desplegada (Netlify):** [https://template-ubits.netlify.app/ubits-colaborador/tareas/tareas.html](https://template-ubits.netlify.app/ubits-colaborador/tareas/tareas.html)

---

*Última actualización: ítems 1–20 (11 feb), 21–26 (19 feb), 27 (Cargar más), 28–30 (20 feb), 31 (21 feb), 32 (23 feb), 33 (nueva versión task-detail), 34 (dropdowns Estado y Prioridad en task-detail), 35 (panel de comentarios: adjuntar imágenes y archivos), 36 (implementación de subtask-detail), 37 (homologación encabezados plan-detail con task-detail), 38 (scroll-spy calendario horizontal, 10 mar), 39 (doble clic para cambio de nombre en tirillas y opción Cambiar nombre en subtareas), 40 (recordatorio registrado en historial en task-detail), 41 (plan-detail: asignados, filtro por persona, filtros aplicados y UI, 11 mar), 42 (task-detail: vencimiento sm, progress bar subtareas, botón Filtrar, Historial de cambios, nueva versión formulario subtareas, 12 mar), 43 (marca "- editado" en comentarios editados, 12 mar), 44 (filtro Asignado específico en tareas, 18 mar), 45 (estilos tirilla en celulares, 18 mar), 46 (Añadir colaborador seguimiento, 20 mar), 47 (resumen multiselect dropdown, 20 mar), 48 (añadir tarea/subtarea encima de listas y orden nuevo arriba, 25 mar), 49 (coachmark Seguimiento: copy, URLs y hash, 25 mar), 50 (coachmark oficial multi-página, 25 mar), 51 (task-detail: tipo Aprendizaje, contenido, icono, 31 mar), 52 (planes: drawer Nueva tarea, paridad Aprendizaje + xs y fecha 105px, 31 mar), 53 (task-strip variante Aprendizaje + id prototipo 9000000000001, 31 mar), 54 (task-detail Aprendizaje: progreso vs finalizada y ocultar buscador, 1 abr), 55 (task-detail Aprendizaje: toasts al pulsar Estado, 1 abr), **56 (Mejoras en la vista de tareas: filtros y tareas sin fecha — attention badge, espaciado scroll, 15 abr)** implementados.*
