...
## Layouts de página (dos de producto + documentación)

Elige **una** familia por pantalla. En producto solo hay **Workspace** e **Inmersivo** (igual que React).
...
- **Input** - Campos de entrada (11 tipos: text, email, password, number, tel, url, select, textarea, search, autocomplete, calendar; tamaños: sm, md, lg; estados: default, hover, focus, invalid, disabled; etiqueta arriba o a la izquierda con `labelPosition: 'left'`; con iconos, contador, helper text, mandatory/optional, validación manual, scroll infinito automático) - **RENDERIZADO: createInput()** — preview de tipos también incluye **Duration** (`createDurationInput()`).
- **Duration input** - Duración mm:ss con “:” fijo (no editable), look del Input (tamaños xs–lg, ícono reloj, helper, label top/left) - **RENDERIZADO: createDurationInput()** — `duration-input.css` + `duration-input.js`; preview en `documentacion/componentes/input.html`.
...
- **Learn content imagen y tráiler** - Bloque de aprendizaje para cargar imagen y/o tráiler en video (vacío con CTA IA «Agregar portada», hover, error, imagen, imagen + IA, tráiler con play) - **RENDERIZADO: HTML + `learn-content-img-trailer.css`; JS opcional `initLearnContentImgTrailer()`**
